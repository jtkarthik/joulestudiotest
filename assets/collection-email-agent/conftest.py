...
def pytest_sessionfinish(session: pytest.Session, exitstatus: int) -> None:
    ...
    for _marker, label, key in SECTIONS:
        ...
        section = {"name": label, "marker": key, "total": total, "passed": passed, "failed": failed, "skipped": skipped, "score": round(passed / total * 100, 2) if total else 0.0, "tests": tests}
        if key == "agent_tests":
    ...
    report_path.write_text(json.dumps({"summary": {"total": total_all, "passed": passed_all, "failed": failed_all, "score": overall_score}, "sections": sections_out}, indent=2))
